 ...
 *     function {@link #parse(InputStream, String)} does not use the XSD file,
 *     but expects the latter to be respected.
 * </p>
 *
...
public class XMLValidationSetParser implements ValidationSetParser {
    ...
    @Override
    public ValidationSet parse(final InputStream stream, final String defaultTitle) throws ValidationSetParseException, NullPointerException {
        // Ensure it may be an XML document:
        Objects.requireNonNull(stream, "Missing input validation set!");
        checkFormat(stream);
        ...
        try {
            ...
            final QueriesHandler querieshandler = new QueriesHandler(defaultTitle);
     ...
     *
     * @throws ValidationSetParseException   In case of error when accessing the stream.
     */
    public void checkXML(InputStream stream) throws ValidationSetParseException {
        if (stream == null)
     ...
     * @param stream    Stream to test.
     *
     * @throws ValidationSetParseException   If the given stream does not contain an XML
     *                          document.
     */
    protected void checkFormat(final InputStream stream) throws ValidationSetParseException {
        if (stream != null && stream.markSupported()){
            try {
                ...
                if (nbRead < 3 || !patternXMLTag.matcher(head).find()){
                    throw new ValidationSetParseException("Unsupported ADQL validation set format! Expected: XML document.");
                }
            }
     ...
     *
     * @throws ValidationSetParseException   If the given stream does not match the expected
     *                          XSD schema.
     */
    protected void validateXMLSchema(final InputStream stream) throws ValidationSetParseException {
        try {
            ...
            throw new ValidationSetParseException(ioe);
        }
    }
    ...
    protected static class QueriesHandler extends DefaultHandler {
        ...
        public final ValidationSet queries;
        ...
        private final StringBuilder eltContent = new StringBuilder();

        public QueriesHandler(final String defaultSetTitle) {
            queries = new ValidationSet(defaultSetTitle);
        }
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
             ...
             */
                ...
                queries.setContact(new Contact());
             ...
             */
                ...
                queries.setPublisher(new Publisher());
             ...
             */
                ...
                switch(ctx.peek()){
                    case QUERIES_FUNCTIONS, QUERY_FUNCTIONS:
                        func = null;
             ...
             */
                ...
                if (attrValue == null)
                    info("Omitted value for the attribute 'uuid'. Automatically generated and set to: '"+q.getId()+"'.");
            ...
            else if (qName.equalsIgnoreCase("adql") && ctx.peek() == Context.QUERY)
            {
                ...
                q.setExpectedToBeValid(Boolean.parseBoolean(attrValue));
                if (attrValue == null)
                    ...
                    warning("Unsupported value for the attribute 'valid': '"+attrValue+"'! Set by default to: '"+q.isExpectedToBeValid()+"'.");

                // set the desired ADQL version:
                attrValue = attributes.getValue("version");
                q.setADQLVersion(ValidationSetParser.parseVersion(attrValue));
                if (attrValue == null)
                    info("Omitted value for the attribute 'version'. Set by default to: '"+q.getADQLVersion()+"'.");
                else if (!ValidationSetParser.matchesVersion(attrValue))
                    warning("Unsupported value for the attribute 'version': '"+attrValue+"'! Set by default to: '"+q.getADQLVersion()+"'.");
        ...
        public void endElement(String uri, String localName, String qName) {
            ...
            switch(ctx.pop()){
                case QUERIES:
                    ...
                    break;
                case QUERIES_TITLE:
                    queries.setTitle(content);
                    break;
                case QUERIES_DESCRIPTION:
                    queries.setDescription(content);
                    break;
                ...
                case FUNCTION_DESCRIPTION:
                    func.setDescription(content);
                    break;
                case FUNCTION_FORM:
                    try {
                        func = UDFParser.parse(content);
                    }catch(UDFParseException upe){
                        error("UDF ignored! Incorrect UDF definition: '"+content+"'. Cause: "+upe.getMessage());
                    }
                    break;
                case FUNCTION:
                    if (func == null)
                ...
                case QUERY_ADQL:
                    ...
                    break;
                case QUERY:
                    if (q.getQuery().isPresent())
                        queries.add(q);
                    else
                        warning("Missing ADQL query for the validation query '"+q.getId()+"'!");
                    q = null;
                    break;
                case CONTACT:
                ...
                case CONTACT_URL:
                    try {
                        final URL contactUrl = new URL(content);
                        queries.getContact().ifPresent(contact -> contact.url = contactUrl);
                ...
                case PUBLISHER_URL:
                    try {
                        final URL publisherUrl = new URL(content);
                        queries.getPublisher().ifPresent(publisher -> publisher.url = publisherUrl);
                    } catch (MalformedURLException e) {
